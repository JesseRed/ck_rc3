function [ output_args ] = ck_rc3_visualize_network_clusterbehavior(outdir,G)
...
% if isfield(handles,'current')
%     if isfield(handles.current,'clusternames')
%     clusternames = handles.current.clusternames;
%     end
%     if isfield(handles.current,'show_idx')
%     if handles.current.show_idx ==3 || handles.current.show_idx ==4
%         R2_3d = handles.G.G2.RiN_3d;
%         R1_3d = handles.G.G1.RiN_3d;
%     else
%         R2_3d = handles.G.G2.R_3d;
%         R1_3d = handles.G.G1.R_3d; 
%     end
...
% end
% if handles.current.show_idx ==3 || handles.current.show_idx==4
%     clusternames = handles.networknames;
% end
...
if group==1
    ...
    titlestr = [ clusternames{pos(2)}, clusternames{pos(1)}, '  R = ', num2str(handles.RR1(pos(1),pos(2),handles.current.behav_idx)), ...
        '  p= ', num2str(handles.RP1(pos(1),pos(2),handles.current.behav_idx))];
    title(titlestr);
    ...
    ylabel(handles.current.name2);
    titlestr = [clusternames{pos(2)}, clusternames{pos(1)}, '  R = ', num2str(handles.RR2(pos(1),pos(2),handles.current.behav_idx)), ...
        '  p= ', num2str(handles.RP2(pos(1),pos(2),handles.current.behav_idx))];
    title(titlestr);
...
handles.G = G;
handles.clusternames = G.clusternames;
handles.clusternames_org = G.clusternames_org;
if isfield(G, 'networknames')
    handles.networknames = G.networknames;
else
    handles.networknames = [];
end
   handles.R1_3d = handles.G.G1.R_3d;
   handles.R2_3d = handles.G.G2.R_3d;
   load(fullfile(outdir,'C2C_stat.mat'));
   handles.C2C_stat = C2C_stat;
   handles.Zp = handles.C2C_stat.Zp;
   handles.RR1 = handles.G.G1.RR;
   handles.RR2 = handles.G.G2.RR;
   handles.RP1 = handles.G.G1.RP;
   handles.RP2 = handles.G.G2.RP;
...
handles.show_var_list = {'conn vs. behav'; 'brain conn';'network vs. behav';'network conn'};
...
assignin('base','y1',y1);
assignin('base','y2',y2);
fprintf('show subject conn fkt ende\n');
end
...
if handles.current.show_idx == 3 || handles.current.show_idx == 4
   handles.clusternames = handles.networknames; 
   handles.R1_3d = handles.G.G1.RiN_3d;
   handles.R2_3d = handles.G.G2.RiN_3d;
   handles.Zp = handles.C2C_stat.Network2.ZiNp;
   handles.RR1 = handles.G.G1.RiNR;
   handles.RR2 = handles.G.G2.RiNR;
   handles.RP1 = handles.G.G1.RiNP;
   handles.RP2 = handles.G.G2.RiNP;  
   
%    if handles.current.show_idx ==4
%        handles.current.M = handles.C2C_stat.Network2.MeanZiNdiffq;
%    end
%    if handles.current.show_idx ==3
%        handles.current.M = squeeze(handles.G.RiNR12(:,:,handles.current.behav_idx));
%    end
       
   %R1 = G.G1.R_3d;
%R2 = G.G2.R_3d;
end
if handles.current.show_idx == 1 || handles.current.show_idx == 2
   handles.clusternames = handles.clusternames_org; 
   handles.R1_3d = handles.G.G1.R_3d;
   handles.R2_3d = handles.G.G2.R_3d;
   handles.Zp = handles.C2C_stat.Zp;
   handles.RR1 = handles.G.G1.RR;
...
if strcmp(str,'not main')
    ...
    adjust_C2C_figure(handles.clusternames);
end
guidata(gcf,handles);
...
if handles.current.show_idx ==2
    %handles.current.clusternames = handles.G.clusternames_org;
...
% korrelation to behavioral data
...
% Netzwerk behavioral data
if handles.current.show_idx ==3
    % hier das ausgewaehlte image rein
       % handles.current.clusternames = handles.G.networknames;
    switch handles.current.conn_string
        case 'MeanZdiff'
                       fprintf('show idx = 3 behve matrix gewaehlt\n');
            %        handles.imagesc = imagesc(handles.C2C_stat.MeanZdiff);
            handles.current.name1 = 'Pearson Mean Z Diff';
            handles.current.name2 = 'Pearson r-value';
            handles.current.M = squeeze(handles.G.RiNR12(:,:,handles.current.behav_idx));
        ...
        case 'MeanSyndiff'
            ...
            %         handles.current.name1 = 'Synchronization';
            %         handles.current.name2 = 'Kopplungsindex';
            %         handles.current.M = squeeze(handles.G.SyniNR12(:,:,handles.current.behav_idx));
        case 'MeanPLVdiff'
            fprintf('not implemented')
            %         handles.current.name1 = 'Phase Locking Value';
            %         handles.current.name2 = 'Phase Locking Value';
            %         handles.current.M = squeeze(handles.G.PLViNR12(:,:,handles.current.behav_idx));
        otherwise
    end
end
...
if handles.current.show_idx ==4
    ...
    switch handles.current.conn_string
        case 'MeanZdiff'
            fprintf('show idx = 4 conn matrix gewaehlt\n');
            %        handles.imagesc = imagesc(handles.C2C_stat.MeanZdiff);
            handles.current.name1 = 'Pearson Mean Z Diff internetwork';
            handles.current.name2 = 'Pearson r-value internetwork';
            handles.current.M = handles.C2C_stat.Network2.MeanZiNdiffq;
            %handles.current.imageMatrixConn = handles.G.Mean;
