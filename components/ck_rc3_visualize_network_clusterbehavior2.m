function [ output_args ] = ck_rc3_visualize_network_clusterbehavior2(outdir,C2C_stat,B)
...
handles.table = uitable('Parent', f, 'Data', cell(4,6),...
       'Units','normalized',...
       'Position', [0.78 0.135 0.2 0.145 ],...
       'ColumnName',{'MeanRdiff','CI','MeanZdiff','CI', 'Zstd', 'p-value'},...
       'ColumnEditable',true,...
       'ColumnWidth',{ 50 70 70 70 },...
       'CellSelectionCallback',@(src,evnt)set(src,'UserData',evnt.Indices));
...
fprintf('<<< FUNCTION ENDE  delete_old_rect() \n');
...
fprintf('>>> FUNCTION START  show_table(pos)... \n');
handles = guidata(gcf);
% beim wechsel von cluster zu netzwerk kann die pos zu gross sein
...
%     r1corr
    ...
    table_data{1,3} = p1corr;
fprintf('show_table 555433\n');
    table_data{2,1} = r2corr;
