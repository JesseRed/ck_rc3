...
if abs(x-xyz(1))>0.01 && stat % edited by ck on 20200525 to avoid the error message by 1.4 mm resolution
...
% auskommentiert am 25.05.2020
% xyz = [xi yi zi 1] * inv(V(1,1).mat') ;
% x = round(xyz(1));
% y = round(xyz(2));
% z = round(xyz(3));
% if abs(x-xyz(1))>0.61 && stat % edited by ck on 20200525 to avoid the error message by 1.4 mm resolution
%     fprintf('fehler in ck_rc2_mni2nifti_real\n');
%     fprintf(' x = %.1f   y= %.1f  z = %.1f \n',x,y,z);
%     fprintf(' xyz(1) = %.1f   xyz(2)= %.1f  xyz(3)in = %.1f \n',xyz(1),xyz(2),xyz(3));
%     fprintf(' xin = %.1f   yin= %.1f  zin = %.1f \n',xi,yi,zi);
