function [ci, plv] = ck_rc3_get_synchronization_ci_PLV(x,y,bin)
%Phase Synchronization 

% x=x-mean(x);
% y=y-mean(y);
% 
% data_length=max(size(x));
% 
% h1=Hilbert(x);
% phase1=phase(h1);
% h2=Hilbert(y);
...
% 
% modphase = mod(phased,2*pi);
% histphase = hist(modphase,bin)/data_length;
% H=ck_dentropy(histphase);
% % Coupling index -> 1=coupled, information=0,  ->(log2(bin)-H)/log2(bin) (Vorschlag DH 24.1.2012) 
% ci=(log2(bin)-H)/log2(bin);
% % subplot(4,1,1);
% plot(dataset1,'b'); hold on; plot(dataset2,'g'); hold off;
% title(['Phase synchronization study'],'fontsize',16);ylabel(['time series'])
% subplot(4,1,2);
% plot(phase1,'b'); hold on; plot(phase2,'g'); plot(phased,'r'); hold off;ylabel(['phases'])
% subplot(4,1,3);
% plot(modphase,'r');ylabel(['phase diff mod'])
% subplot(4,1,4); 
% bar(histphase);ylabel(['Hist(phase diff)']);
% xlabel(['entropy= ',num2str(H),'                coupling index= ',num2str(coupling_index)],'fontsize',16)
...
modphase = mod(phased,2*pi);
histphase = hist(modphase,bin)/length(x);
H=ck_rc3_dentropy(histphase);
% Coupling index -> 1=coupled, information=0,  ->(log2(bin)-H)/log2(bin) (Vorschlag DH 24.1.2012) 
ci=(log2(bin)-H)/log2(bin);
% subplot(4,1,1);
plv =  abs(sum(exp(1i* modphase))/length(modphase));
